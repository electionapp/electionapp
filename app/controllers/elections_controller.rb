require 'csv'class ElectionsController < ApplicationController
 before_action :require_login
 before_action :set_election, only: [:show, :edit, :update, :destroy]  # GET /elections
 # GET /elections.json
 def index
   @elections = current_user.elections
 end  def show
   @election = Election.find(params[:id])
 end  # GET /elections/new
 def new
   @election = Election.new
 end  # GET /elections/1/edit
 def edit
   @election = Election.find(params[:id])
 end  # POST /elections
 # POST /elections.json
 def create
   ...
   if(!params[:users].nil? && !params[:users][:csv].nil?)
     ...
     myFile = params[:users][:csv] #The csv file the user uploaded      #Loop over each user in the file
